...
// カウントダウンタイマー
const isAfter15 = new Date() >= new Date("2026/04/01 00:00:00");
...
let countdown = setInterval(function () {
  ...
  const target = new Date("2026/3/31 23:59:59"); //ターゲット日時を取得
...
let countdown02 = setInterval(function () {
  const now = new Date(); // 今の日時
  const target = new Date("2026/4/15 23:59:59"); // ターゲット日時を取得
  let remainTime = target - now; // 差分を取る（ミリ秒で返ってくる）
