...
//  front-page.phpで読み込まれているものと被るのでコメントアウト
...
function add_mwform_validation_rule($Validation, $data) {
  ...
  //add_filter('mwform_validation_mw-wp-form-6', 'add_mwform_validation_rule', 10, 2);
...
function autoback_my_mail_family($Mail_raw, $values, $Data)
{
	if ($Data->get('radio-name') == 'ガーデンテラス東山') {
...
add_filter('mwform_admin_mail_mw-wp-form-1334', 'autoback_my_mail_family', 10, 3);
...
add_filter( 'document_title_parts', 'custom_lp_title' );
function custom_lp_title( $title ) {
    // スラッグが '/lp' の場合、タイトルを「豪華結婚式無料プレゼント！ | otokucon」に変更
    if ( is_page() && get_queried_object()->post_name === 'lp' ) {
        $title['title'] = '豪華結婚式無料プレゼント！ | otokucon';
    }
    return $title;
}
...
function add_replaceState_hook_script() {
    wp_register_script(
        'replaceState-hook',
        get_template_directory_uri() . '/assets/js/replaceState-hook.js?1059',
        array(), // 依存なし
        null,
        false // head に挿入
    );
    wp_enqueue_script('replaceState-hook');
}
add_action('wp_enqueue_scripts', 'add_replaceState_hook_script', 1);

/**
 * canonicalリダイレクトを無効化して、すべてのクエリパラメータを残す
 */
add_filter('redirect_canonical', 'disable_canonical_redirect_for_front_with_query', 10, 2);
function disable_canonical_redirect_for_front_with_query($redirect_url, $requested_url) {
    // トップページかつGETパラメータがあるときはリダイレクトを無効化
    if ((is_front_page() || is_home()) && !empty($_GET)) {
        return false;
    }
    return $redirect_url;
}
